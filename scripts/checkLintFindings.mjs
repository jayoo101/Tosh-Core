 ...
 * `docs/SECURITY_AUDIT.md` §2.5 cannot drift away from the code it describes.
 *
...
if (UPDATE) {
  const doc = {
    _readme:
      'Generated by scripts/checkLintFindings.mjs --update. Every entry here must have ' +
      'its bound recorded in docs/SECURITY_AUDIT.md §2.5. Regenerating this file without ' +
      'updating that section is the drift the guard exists to prevent.',
...
for (const [k, f] of have) {
  if (!want.has(k)) {
    fail.push(
      `NEW finding, and §2.5 does not carry its bound:\n        ${show(f)}\n` +
        '        Record what bounds it in docs/SECURITY_AUDIT.md §2.5, then rerun with --update.',
    );
  ...
  if (!have.has(k)) {
    fail.push(
      `finding GONE, so §2.5 now describes code that is not there:\n        ${show(f)}\n` +
        '        Delete its row from docs/SECURITY_AUDIT.md §2.5, then rerun with --update.',
    );
  }
}

// ── 4. The dossier must state the same totals ───────────────────────────────
//
// The point of the baseline is that §2.5 is true. Pinning the lint output
// while the prose keeps its own stale arithmetic would reproduce the original
// bug one layer down.
//
// The dossier names these findings the way a reader would — "18
// `block.timestamp` findings" — rather than by `forge lint`'s rule id, so each
// rule carries the aliases the prose is allowed to use. EVERY "<number>
// <alias>" in the document must agree with the tool; matching in one place
// while contradicting itself in another is the same defect one paragraph over.

const ALIASES = {
  'unsafe-typecast': ['unsafe-typecast'],
  'block-timestamp': ['block-timestamp', 'block\\.timestamp'],
};

if (fs.existsSync(DOSSIER)) {
  const md = fs.readFileSync(DOSSIER, 'utf8');
  for (const [rule, n] of Object.entries(totals)) {
    const aliases = ALIASES[rule];
    if (!aliases) {
      note.push(`no prose alias registered for rule '${rule}' (actual: ${n}); cross-check skipped.`);
      continue;
    }
    const re = new RegExp(String.raw`(\d+)\s+\x60?(?:${aliases.join('|')})\x60?`, 'g');
    const stated = [...md.matchAll(re)].map((m) => Number(m[1]));
    if (stated.length === 0) {
      fail.push(
        `docs/SECURITY_AUDIT.md never states how many '${rule}' findings there are (actual: ${n}).\n` +
          '        §2.5 is the auditor\'s map of these; a map with no scale is not one.',
      );
    } else {
      const wrong = [...new Set(stated.filter((s) => s !== n))];
      if (wrong.length > 0) {
        fail.push(
          `docs/SECURITY_AUDIT.md states ${wrong.join(' and ')} for '${rule}'; forge lint reports ${n}.\n` +
            '        This is the exact drift this guard exists for. Fix the prose in §2.5.',
        );
      }
    }
  }
} else {
  note.push('docs/SECURITY_AUDIT.md not found; skipped the prose cross-check.');
}
...
console.log('      Every one has its bound recorded in docs/SECURITY_AUDIT.md §2.5.');
