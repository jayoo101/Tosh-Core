...
## 0-bis. 这次部署已于 2026-09-12 被取代

**下面 §0 描述的那对合约不再是平台。** 2026-09-12 又广播了一次,因为 8%/2% 的
推荐分成落在 `immutable` 上,改不了,只能重新部署。写在这里而不是去改 §0,是因为
§0 是一份带日期的记录:它对 2026-09-08 那天的描述是准确的,把地址替换掉只会同时
毁掉两份记录,而这份手册的价值恰恰在于它记的是当天实际怎么走的。

| | 2026-09-08(§0,已退役) | 2026-09-12(现行) |
|---|---|---|
| 工厂 | `0xBa9d2E86281b988225Eca383C375215912fb20B9` | `0x2920ca7E9fcD85491D699e1f9Ae2CAa65Cfb2892` |
| 金库 | `0x99aD248dD15498957B864Fd79917F0E103Aa78F7` | `0x255722226720914eF5B2CD54647f21f584BD4Ea2` |
| 区块 | 57400516 | 61056709 |
| 提交 | `d0220e2` | `9b9d9ce`,在 `main` 上叫 `d18d2d5` |

**没变的东西比变了的更值得注意。** 所有者 Safe 仍是
`0x2953957774482efA660921df85A1E7634ccfe27A`,池管理器、PoG 签名者、平台金库地址
都没动,部署者仍是 `0x4E41CEa950cF40FA59774B409988D6F9F399E690`。§§1–6 的流程
一字未改地又走了一遍,包括 §5 那个 `set -a`。所以这份手册作为操作顺序仍然有效,
过期的只有 §0 的那张状态表。

**必须重做而且做完了的:** PM-C2 在两个新合约上各调一次 `acceptOwnership()`,
两者 `owner()` 现在都是 Safe、`pendingOwner()` 都是零地址;
`HOOK_CREATION_CODEHASH` 重新测量为
`0xece3b0c259085201dda4d6a2ab94ed469a1d806b498a4d8c411b74ecd0b549f4`,
`RecomputeInitcodeHash.s.sol` 干净退出即为断言通过;`.env.production` 四个值
全部换新。

**两处代价要记下来,因为它们不是错误而是选择的后果。** 一是每个用户的 PoG 认证
随新工厂重置 —— 认证是绑在工厂地址上的,旧的在新工厂上不成立。二是 2026-09-08
那两个测试项目连同它们的池子留在旧工厂上,不迁移,前端也不再列出它们;它们没有
坏掉,只是不再属于这个平台。`SECURITY.md` 把旧的一对明确标为 out of scope。

**§0 之后所有以日期署名的条目(PM-C4、PM-C6、PM-C7、PM-C8……)照原样保留。**
它们记录的是那次部署上发生过的事,而那件事确实发生过。凡是在新部署上重新做过的,
上面这一节说明了;凡是没说的,就是还没在新部署上重做 —— 尤其 PM-C7 那条"从未有人
在真实部署上点过一次 `/api/pog-scan` 与 `/api/sign-allocation`",在新部署上依然
成立,而且因为 PoG 重置,它现在比之前更要紧。

---

## 0. 2026-09-08 广播之后的状态

> **这一节是那一天的历史记录,里面的工厂和金库地址已于 2026-09-12 退役。**
> 上面第 28 行那张对照表给的才是现行地址。这句话写在这里而不是只写在文件开头,
> 是因为真实的读法不是从头读:是有人搜一个地址、直接落在下面那张表上。
> 2026-09-13 就是这么丢了四天的链上监控 —— `MONITOR_FACTORY` 指着下面这个
> 地址,每一次巡查都是绿的(`SECURITY_AUDIT.md` §5.36)。

| | |
|---|---|
| ~~规范~~ 工厂(**已于 2026-09-12 退役**,现行的见上面那张对照表) | `0xBa9d2E86281b988225Eca383C375215912fb20B9` —— 运行时 10,789 字节,链 4663 |
| ~~规范~~ 金库(**已于 2026-09-12 退役**) | `0x99aD248dD15498957B864Fd79917F0E103Aa78F7` —— 运行时 6,035 字节。互相接线:`factory.ladderTreasury()` 和 `treasury.factory()` 各自返回对方 |
...
8. **PM-E2 —— 重新指向那一半已完成。** 四个 `MONITOR_*` GitHub 变量都持有主网
   ...
   > **上面那段话在 2026-09-11 是真的,2026-09-12 变成了假的,而没有任何东西
   > 说过一声。** 重新部署换了工厂和金库,那两个变量没跟着换,于是接下来四天
   > 巡查报的全是退役合约 —— 而且每次都绿,因为退役工厂照样用 Safe 回答
   > `owner()`、用 false 回答 `paused()`、什么事件都不发。2026-09-13 修好并
   > 补扫,那段窗口里躺着 8 条从未被观测的链上事件。
   >
   > **这条注记本身就是这次事故的教训所在:一句"核对过"不带失效条件,就是一把
   > 会过期而不报废的锁。** 任何"某某已核对"的记录都必须同时写下什么会让它作废
   > —— 这里是"下一次重新部署"。守卫在 `WATCHER-06`,叙述在
   > `SECURITY_AUDIT.md` §5.36。同一节还记了另一件事:改对这两个变量本身
   > 会触发一条假的 P0 掏空告警,那需要的是根治而不是守卫。

   ~~仍然未做的不是代码:**一个 P0 落地的地方仍然是一个 issue,而不是一部手机。**~~
   **这句话在 2026-09-11 起不再成立** —— `report.mjs` 当天加了 Telegram 推送并
   做过演练(`ONCHAIN_MONITORING.md` §8)。剩下的不是投递而是**节奏**:实测
   GitHub 每天只发约 6.5 次定时运行,把 cron 从每小时改成每 15 分钟一次都没多
   (0.269 次/小时 对 0.27 次/小时),所以真实检测延迟是中位数约 3.3 小时、
   最坏 7.2 小时。`WATCHER-05` 现在每次巡查都把这个间隔打出来。
   第一次主网巡查是瞎的 ——
