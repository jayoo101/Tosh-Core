...
>
> **取证方式**：纯静态阅读，未执行 `forge build` / `forge test`。所有数字、常量、函数名都标注了来源文件与行号（格式 `src/ToshLaunchpadHook.sol:247`）。行号对应本次阅读时的文件状态；如有并发修改，请以 `git diff` 复核。
>
> **标注约定**：
> - **⚠️ 需确认** —— 代码里客观存在，但可能与产品直觉不符的地方。全部条目在第 8 章汇总。
> - **代码中未找到依据** —— 我无法在代码里找到支撑的内容，不做编造。
>
> **审阅范围**：`src/`（4 个核心合约 + 2 个库）、`script/`（5 个部署脚本）、`test/`（14 个测试文件，其中 `ToshV5*.t.sol` 是 v5.0 的验收套件）、`soat-frontend/`（Next.js dApp）。
...
| 2 | 项目方持有大额预挖，区块浏览器上一个地址占 100% 供应，社区不敢进 | 按需铸造（on-demand mint）。开盘时只铸 8.4M 创世块，其余 12.6M 随货架成交逐笔铸出 | `src/ToshToken.sol:35-39`、`src/ToshToken.sol:111-114` |
...
| 4 | 创世参与者开盘就被套（开盘价 ≤ 出资成本） | 55/45 的创世供应切分在数学上制造出**恰好 10%** 的开盘账面溢价，货架 0 再叠 5%，合计 15.5% | `src/ToshLaunchpadHook.sol:141-162`；测试 `test_genesisPremium_isExactlyTenPercent` @ `test/ToshV5.t.sol:595` |
| 5 | 底池被永久锁死，散户不敢也不能做 LP，池子费收给了无人能领的仓位 | v5.0 从地址掩码里**移除** `BEFORE_REMOVE_LIQUIDITY`（0x22CC → 0x20CC），散户 LP 自由进出；创世仓位靠"归属权 + 无移除代码路径"结构性锁定 | `src/ToshLaunchpadHook.sol:73-88`、`1018-1042`；测试 `test_retailLp_canAddAndRemoveWithoutTouchingGenesis` @ `test/ToshV5.t.sol:633` |
| 6 | 通缩靠链下机器人 harvest，有 MEV 风险和运维成本 | 顺风车（piggyback）回购：任何一笔 Tosh 池交易的 `afterSwap` 都会顺手把平台国库的 1 ETH 拆成 3 份轮转买入并销毁到 `0xdead`，无链下组件 | `src/ToshLaunchpadHook.sol:1142-1157`；`src/ToshLadderTreasury.sol:267-304` |
...
- **卖压直接销毁**：卖出侧 0.7% 的代币在 `beforeSwap` 里就被 `take` 到 `0xdead`，不进储备、不需要回购换手（`src/ToshLaunchpadHook.sol:1113-1116`）。
...
```
   ...
   │ · deposit 创世网关        │──────────────────────▶│ · autoPiggybackBuyback         │
   │ · globalReferrers 推荐图  │                       │ · _buyAndBurn → 0xdead         │
   │ · 黑名单 / 冷却 / 暂停    │                       │ · piggybackActive() 瞬态锁      │
   └──────────┬───────────────┘                       └───────────┬───────────────────┘
              │ CREATE2 部署（经 HookDeployLib delegatecall）        │ swap/settle/take
              │ + initializeToken                                  │（在别人的 unlock 帧里）
              ▼                                                    ▼
   ┌──────────────────────────────────────────────────┐   ┌──────────────────────┐
...
| 2 | 部署者 → Factory | `constructor(poolManager, pogSigner, platformTreasury, ladderTreasury)` | 同时算出 `HOOK_CREATION_CODEHASH` | `src/ToshFactory.sol:199-213` |
| 3 | owner → Treasury | `setFactory` | **一次性**，闭环。未闭环则所有回购静默失效 | `src/ToshLadderTreasury.sol:173-178` |
| 4 | creator → Factory | `createLaunch{value: fee}` | CREATE2 部署 hook → `new ToshToken` → `token.initialize(hook)` → `hook.initializeToken(token)` | `src/ToshFactory.sol:346-422` |
| 5 | 储户 → Factory → Hook | `deposit{value}` → `hook.deposit(user, boundReferrer)` | 资格校验全在工厂，记账全在 hook | `src/ToshFactory.sol:442-468` → `src/ToshLaunchpadHook.sol:624-654` |
| 6 | creator → Hook → PoolManager | `launch()` → `initialize` + `unlock` → `unlockCallback` → `modifyLiquidity` | 建池 + 注入全区间创世流动性 | `src/ToshLaunchpadHook.sol:704-766`、`1224-1262` |
| 7 | 任意交易者 → PoolManager → Hook | `beforeSwap` / `afterSwap` | 抽税 + 写预言机 + 顺风车 poke | `src/ToshLaunchpadHook.sol:1087-1160` |
| 8 | Hook → Treasury | `autoPiggybackBuyback()`（`try/catch` 包裹） | 故障隔离：国库炸了不能让交易炸 | `src/ToshLaunchpadHook.sol:1154-1157` |
| 9 | Treasury → PoolManager | `swap` / `sync` / `settle` / `take` | **不调 `unlock`**，因为已在别人的 unlock 帧内 | `src/ToshLadderTreasury.sol:53-60`、`327-352` |
| 10 | Treasury → Factory | `registeredHooks` / `tokenToHook` | 认证 poke 来源；校验策展代币来源 | `src/ToshLadderTreasury.sol:156-160`、`204-208` |
| 11 | Treasury → Hook | `getPoolKey()` | 回购场地从 hook 反查，**不由 owner 提供** | `src/ToshLadderTreasury.sol:210` |
...
| 调整冷却期 | `setCooldownDuration` | `≤ MAX_COOLDOWN = 7 days`；为 0 时 PoG 额度退化为终身预算（⚠️ 8.25） | `src/ToshFactory.sol:244-248` |
| 调整默认软顶 | `setDefaultSoftCap` | `≥ MIN_SOFT_CAP_PROD = 0.01 ether`，防 `p0` 截断为 0 | `src/ToshFactory.sol:250-256`、`45-55` |
...
| 批量拉黑 / 解禁 | `setBlacklist` / `liftBlacklist` | 每批 ≤ 200；`banDuration == type(uint256).max` 为永久 | `src/ToshFactory.sol:263-278` |
| 策展回购阶梯 | `addLadderToken` / `removeLadderToken` | 代币必须经本平台 `tokenToHook` 注册且已开盘 | `src/ToshLadderTreasury.sol:200-254` |
| 绑定国库↔工厂 | `setFactory` | 一次性，不可重指 | `src/ToshLadderTreasury.sol:173-178` |

| **不能做** | 原因 |
|---|---|
| 从国库取走任何 ETH | 国库无 `withdraw`/`sweep`/`rescue`/`delegatecall`；唯一出金 `_buyAndBurn` 收款方硬编码 `0xdead`（`src/ToshLadderTreasury.sol:37-51`）。测试 `test_ladderTreasury_hasNoWithdrawPath` @ `test/ToshV5.t.sol:1196` |
| 把回购资金导向自己控盘的池子 | `addLadderToken` 只收本平台已开盘代币，池子 key 从 hook 反查（`src/ToshLadderTreasury.sol:180-226`）。测试 `test_ladderTreasury_ownerCannotRedirectSpendToOwnPool` @ `test/ToshV5.t.sol:1216` |
...
- **是**：`createLaunch` 的 `msg.sender`，被写入 hook 的 `creator` immutable（`src/ToshLaunchpadHook.sol:324`）。
- **唯一专属权限**：调用 `launch()` 开盘（`src/ToshLaunchpadHook.sol:705`，`OnlyCreator`）。
- **不能**：改 `projectAdmin`（那是 `projectAdmin` 自己的权限）、不能提前开盘（必须 `block.timestamp >= genesisDeadline`，⚠️ 8.7）、不能退款给自己、不能移除创世流动性。
- 参与 CREATE2 盐派生：`finalSalt = keccak256(abi.encode(msg.sender, hookSalt))`（`src/ToshFactory.sol:367`），所以**盐是创作者绑定的**，别人挖到的盐在你身上无效。

#### 2.2.3 项目管理员（projectAdmin）

| 能做 | 函数 | 行号 |
|---|---|---|
| 领取 Phase-2 货架收入的 99% | 被动接收（`mintBondingCurve` 内 `_sendEth`） | `src/ToshLaunchpadHook.sol:914-918` |
| 把角色移交给新钱包 / 多签 | `changeProjectAdmin(newAdmin)` | `src/ToshLaunchpadHook.sol:603-610` |
...
- 构造参数、`require` 非零、写入 immutable（`src/ToshLaunchpadHook.sol:325`、`546`、`559`）。
...
- 工厂的可变 owner 状态（`src/ToshFactory.sol:74`、`233-237`）。
- **⚠️ 8.2：没有任何资金流向它。** 发射费走 `ladderTreasury`（`src/ToshFactory.sol:416-419`），Phase-2 的 1% 平台切片也走 `ladderTreasury`（`src/ToshLaunchpadHook.sol:917`）。`platformTreasury` 在代码里只被 `getLiveHookInitcodeHash` 当作哨兵地址填充占位（`src/ToshFactory.sol:534`）。

#### 2.2.6 创世储户

| 能做 | 何时 | 函数 | 行号 |
|---|---|---|---|
| 出资 | 创世窗口内、有 PoG 额度、未拉黑、未冷却、未超每钱包上限 | `factory.deposit{value}(hook, referrer)` | `src/ToshFactory.sol:442-468` |
| 100% 退款 | 软顶未达成，或超 7 天僵尸窗口 | `hook.refund()` | `src/ToshLaunchpadHook.sol:668-693` |
| 按出资比例认领 4,620,000 枚中的份额 | 开盘后，一次性 | `hook.claimGenesis()` | `src/ToshLaunchpadHook.sol:769-781` |

- **不能**：多次认领（`genesisShareClaimed` 布尔位）、退款后再认领（`refund` 要求 `!launched`，两条路径互斥）、在开盘后退款。

#### 2.2.7 二级交易者

- 在 V4 池子上正常 swap。每笔付 **1.00%** 摩擦：0.30% `POOL_FEE` 归 LP（V4 原生结算），0.70% `TAX_BPS` 归协议（`src/ToshLaunchpadHook.sol:239`、`302`）。
...
- 佣金 = 被推荐人每次创世出资的 10%，在 `deposit` 时即计入 `referralAccrued`，开盘后可提（`src/ToshLaunchpadHook.sol:642-651`、`790-801`）。
- **失败的创世不欠推荐人任何东西**：退款返 100%，佣金只在 `launch()` 时才真正兑现，`referralAccrued` 单纯变成永不可领（`src/ToshLaunchpadHook.sol:664-667`）。

#### 2.2.9 散户 LP

- 用**自己的**（或所经由的 router/posm 的）V4 仓位，自由 `modifyLiquidity` 增减（`src/ToshLaunchpadHook.sol:1018-1042`）。
- 赚 0.30% 池子费，由 V4 原生计入仓位，Tosh 无分配代码。
- **不能**碰创世仓位：V4 把仓位按 `msg.sender` 归属，创世仓位归 hook，而 hook 的 `unlockCallback` 只认 `ACTION_ADD_LIQUIDITY`（`src/ToshLaunchpadHook.sol:1212-1220`）。测试 `test_genesisLiquidityIsPermanentlyLocked` @ `test/ToshV5.t.sol:613`。
...
|---|---|---|---|---|---|
| 硬顶 | `ToshToken.MAX_SUPPLY` | 21,000,000e18 | 100% | `mint` 时逐笔校验 | `src/ToshToken.sol:53`、`112` |
| 创世块 | `GENESIS_SUPPLY` | 8,400,000e18 | 40% | 开盘时一次性铸给 hook | `src/ToshLaunchpadHook.sol:139`、`731` |
| ├ 认领侧 | `GENESIS_CLAIM_SUPPLY` | 4,620,000e18 | 22%（创世块的 **55%**） | 储户按出资比例认领 | `src/ToshLaunchpadHook.sol:161` |
| └ 底池侧 | `GENESIS_LP_SUPPLY` | 3,780,000e18 | 18%（创世块的 **45%**） | 全区间注入底池并永久锁定 | `src/ToshLaunchpadHook.sol:162` |
| Phase-2 阶梯 | `BONDING_MAX = TIER_COUNT × TIER_SIZE` | 12,600,000e18 | 60% | 4000 档 × 3,150 枚 | `src/ToshLaunchpadHook.sol` |

算术闭合：8,400,000 + 12,600,000 = 21,000,000，精确等于硬顶。测试 `test_supplyPartitioning` @ `test/ToshV5Guards.t.sol:458`。
...
代码位置：切佣 `src/ToshLaunchpadHook.sol:642-651`；`lpEth` 计算 `721-723`；孤儿佣金转出 `758-763`；建池注入 `1224-1262`。测试 `test_orphanReferralIsForwardedToLadderTreasuryAtLaunch` @ `test/ToshV5.t.sol:537`。
...
```
p0      = lpEth × 1e18 / GENESIS_LP_SUPPLY            // src/ToshLaunchpadHook.sol:726
shelfP0 = p0 × SHELF_PREMIUM_BPS / 10000
        = p0 × 10500 / 10000
        = p0 × 1.05                                    // src/ToshLaunchpadHook.sol:728
```

**10% 溢价的数学推导**（合约注释里已给出，`src/ToshLaunchpadHook.sol:144-160`）：
...
也就是：**55/45 的切分 + 10% 推荐率，三者共同决定了这个 1.10**。动任何一个，溢价就变。测试用 `assertApproxEqRel(…, 1e12)` 把这个**关系**（而非硬编码价格）钉住，并同时校验 `shelfP0 = 1.155 × 成本基准`（`test/ToshV5.t.sol:595-611`）。

**为什么货架要再高 5%**（`src/ToshLaunchpadHook.sol:249-278`）：
...
- 三个后果：
  1. **开盘瞬间 Phase-2 完全关闭**。开盘时 `spot == p0` 精确落在边界上（`tierPriceAt(0) = 1.05·p0 > 1.05·p0` 不成立，但等号成立时 `≤` 通过——实测 `maxMintable() == 0`，见 `test/ToshV5.t.sol:862`）。若货架与池子齐平，开盘那一刻货架 0..14（126,000 枚）就是可铸的。
  2. **铸币砸盘在整条阶梯上都亏钱**，因为买家永远比市价高付 5%（测试 `test_sweepAndDumpIsLossMaking` @ `test/ToshV5.t.sol:885`，断言亏损 > 成本的 5%）。
...
**为什么步进与档数耦合**：注释明确说明 `STEP = 1000^(1/1999)`，两者必须一起改。且跨度不能任意拉高——`1.2^1999 ≈ 1e158` 会在阶梯清空前就溢出 `uint256`（`src/ToshLaunchpadHook.sol:180-185`）。

**为什么价格不落盘**：2000 档物化成 storage 结构体要花掉数百万 gas；而"缓存当前价 + 累乘推进"经过 2000 次截断会与闭式解漂移。所以 `tierPriceAt()` 是铸造热路径、`quoteMint`、以及所有 view 的**唯一价格来源**（`src/ToshLaunchpadHook.sol:396-403`、`1396-1400`）。`getTiers()` 分页 view 也逐档重算而不是向前走序列，理由是"显示价与成交价差 1 wei 就是一张客服工单"（`src/ToshLaunchpadHook.sol:1459-1463`）。

### 3.5 费率总表

| 费用 | 常量 | 值 | 来源侧 | 去向 | 行号 |
|---|---|---|---|---|---|
| 发射费 | `ToshFactory.launchFee` | 0.1 ETH（默认，owner 可调，可为 0） | creator | `ladderTreasury`（回购弹药） | `src/ToshFactory.sol:78`、`416-419` |
| 推荐佣金 | `REFERRAL_BPS` | 10%（1000 bps） | 每笔创世出资 | 推荐人；无推荐人→`ladderTreasury` | `src/ToshLaunchpadHook.sol:226`、`642-651` |
| Phase-2 平台切片 | `PLATFORM_TAX_BPS` | **1%**（100 bps） | 货架成交额 | `ladderTreasury` | `src/ToshLaunchpadHook.sol:230`、`914-917` |
| Phase-2 项目切片 | 余额 | **99%** | 货架成交额 | `projectAdmin` | `src/ToshLaunchpadHook.sol:915`、`918` |
| 交易税（协议） | `TAX_BPS` | **0.70%**（70 bps） | 每笔 swap 的 **input**（按买/卖方向，不按 specified 币种） | 买单 ETH→`ladderTreasury`；卖单代币→`0xdead` | `src/ToshLaunchpadHook.sol:249`、`beforeSwap`/`afterSwap` |
| 池子费（LP） | `POOL_FEE` | **0.30%**（3000，V4 单位） | 每笔 swap | LP（V4 原生结算，Tosh 无代码） | `src/ToshLaunchpadHook.sol:302`、`288-301` |
| **交易者总摩擦** | — | **1.00%** | — | 0.30 给 LP + 0.70 给协议 | `src/ToshLaunchpadHook.sol:235-238` |

**v4.x → v5.0 的摩擦重分配**：v4.x 收 1% 池子费 + 1% 税 = 2%，而池子费那一半是死重（唯一 LP 是永久锁定的创世仓位，没人能领）。v5.0 把总摩擦砍回 1.00%，并让池子费真正有了领取人（`src/ToshLaunchpadHook.sol:73-84`、`290-301`）。
...
判据代码：`canRefund()` @ `src/ToshLaunchpadHook.sol:657-662`；`launch()` 的四道前置 @ `705-710`；`refund()` 的三道前置 @ `669-674`。

> **⚠️ 8.13**：`refundEnabled` / `zombieRefundEnabled` 两个状态位在 `refund()` 里被懒惰置位并发事件（`src/ToshLaunchpadHook.sol:682-689`），但**从未被任何地方读作门控条件**。真正的门控是每次调用时重算 `softCapFailed || zombieExpired`。它们是纯事件标记位。

### 4.2 创建发射 —— `createLaunch`

**签名**（`src/ToshFactory.sol:346-354`）：
...
**9 参构造元组**（`src/libraries/HookDeployLib.sol:58-71`，顺序固定）：
...
**这 9 个参数全部进 initcode hash**，所以任何一项变化都会让已挖的盐失效。前端为此做了三处缓存失效（见 6.4）。`HookDeployLib` 的注释直接点名：v5.0 删了 `satoToken`、加了 `ladderTreasury` + `perWalletCap`，`genesisDuration` 让它凑到 9 个字段，**所有链下盐矿机必须重新生成**（`src/libraries/HookDeployLib.sol:20-25`）。
...
|---|---|---|---|---|
| `DURATION_FAST` | 3 hours | "3 Hours / Fast" | "Momentum play — hits the cap fast or fails fast." | `src/ToshLaunchpadHook.sol:214` |
| `DURATION_STANDARD` | 24 hours | "24 Hours / Standard" | "Covers every timezone once. The default." | `src/ToshLaunchpadHook.sol:215` |
| `DURATION_SLOW` | 72 hours | "72 Hours / Slow" | "Maximum reach for a wider raise." | `src/ToshLaunchpadHook.sol:216` |
...
**为什么是封闭集合而不是自由 `uint256`**（`src/ToshLaunchpadHook.sol:204-213`）：时长是构造元组的一员，因此也是 initcode hash 的一部分。开放区间会让创作者能针对 1 秒窗口（没人来得及存，创世立刻失败，退款即刻打开）或 100 年窗口（存款被锁死且无退款路径）挖盐。三档粗粒度既保留市场意义，也堵住两个退化端。构造函数逐一比对三个常量，否则 revert `InvalidDuration`（`src/ToshLaunchpadHook.sol:552-555`）。测试：`test_hook_ctor_acceptsTheThreeAllowedWindows` @ `test/ToshV5Guards.t.sol:184`，`test_hook_ctor_revertsOnUnlistedWindow` @ `202-206`，`test_createLaunch_rejectsSaltMinedForAnotherWindow` @ `test/ToshV5Factory.t.sol:928`。

#### 4.3.2 PoG（Proof-of-Gas / Goodwill）配额与签名注册

> 注：合约注释里 `registerPoG` 被称为 "Proof of Goodwill"（`src/ToshFactory.sol:284`），前端与 README 叫 "Proof-of-Gas"。**代码中未找到**统一命名的依据，两个名字都存在。

**签名摘要**（`src/ToshFactory.sol:295-297`），六元组防重放：
...
**校验顺序**（`src/ToshFactory.sol:286-303`）：
...
| `maxAlloc ≤ maxPogAllocationLimit` | `ExceedsGlobalPogLimit` | **不静默截断**，直接拒（测试 `test_registerPoG_noSilentClamp` @ `test/ToshV5Factory.t.sol:213`） |
| 签名恢复 == `pogSigner` | `InvalidSignature` | |

**额度只上调不下调**：`if (maxAlloc > pogQuota[sender]) pogQuota[sender] = maxAlloc;`（`src/ToshFactory.sol:300-302`）。⚠️ **8.10**：owner 事后调低 `maxPogAllocationLimit` **不会**回收已注册的额度。

⚠️ **8.11**：`registerPoG` 有 `whenNotPaused` 但**没有黑名单检查**。被拉黑的钱包仍可注册/提升 PoG 额度（只是 `deposit` 会被 `IsBlacklisted` 拦住）。

**配额窗口机制**（`src/ToshFactory.sol:105-117`、`591-603`）：

- PoG 额度是**冷却期预算**，不是终身预算：一个钱包在每个 `cooldownDuration` 窗口内可花掉最多 `pogQuota`，窗口过期后归零重开。
- **退款不返还窗口额度**——刻意的：撤资就该失去这一轮的名额，否则「存入-退款」循环能无限复用一个钱包的额度（`src/ToshFactory.sol:111-116`）。测试 `test_pogQuota_isNotRestoredByRefund` @ `test/ToshV5.t.sol:425`。
...
- `perWalletCap` 是**单项目的上限**，对照**创建时的快照**执行，让平台事后调旋钮无法改变正在募集中的项目的条款（`src/ToshLaunchpadHook.sol:335-344`）。测试 `test_perWalletCap_isSnapshottedAtProjectCreation` @ `test/ToshV5.t.sol:396`。
...
**创世流动性为何不需要回调就锁死**（`src/ToshLaunchpadHook.sol:1018-1034`）：V4 把每个仓位按调用 `modifyLiquidity` 的地址归属（`Pool.ModifyLiquidityParams.owner = msg.sender`）。创世仓位归 hook，而 hook 的 `unlockCallback` 只识别 `ACTION_ADD_LIQUIDITY`、且 delta 严格为正。任何人（包括 creator、包括 owner）都无法寻址那个仓位。所以锁是**结构性**的，而不是靠一个会 revert 的回调，`BEFORE_REMOVE_LIQUIDITY` 因此从掩码里删除而不是软化成条件 revert。
...
**暂停不影响退款**：测试 `test_pause_doesNotBlockRefund` @ `test/ToshV5Factory.t.sol:445` 明确固化了这一点（⚠️ 8.12 的另一面：这是好事，但也说明暂停覆盖面很窄）。
...
EFFECTS：currentTierIndex / currentTierSold / phase2Minted += tokenAmount
...
把买家能撞到的**所有数量限制**折叠成一个数（`src/ToshLaunchpadHook.sol:976-1000`）：当前档剩余 + 后续所有仍在 105% 天花板下的档 + 阶梯末端 + `MAX_TIERS_PER_TX`。门关着时返回 0。UI 的 "max" 按钮据此定档，而不是猜 `TIER_SIZE`。测试 `test_maxMintable_isTheExactAcceptedBoundary` @ `test/ToshV5.t.sol:789` 断言"多一个 wei-token 就 revert"。
...
| `getTiers(start, count)` | 分页窗口（2000 档 = 6000 字，无法一次返回） | `1444-1466` |
...
### 4.9 平台国库顺风车回购 —— `autoPiggybackBuyback()`

**触发链路**：任何 Tosh 池的任何一笔 swap → hook 的 `afterSwap` → `try IToshLadderTreasury(ladderTreasury).autoPiggybackBuyback()`（`src/ToshLaunchpadHook.sol:1142-1157`）。
...
**国库侧执行**（`src/ToshLadderTreasury.sol:267-304`）：

| 步 | 动作 | 说明 |
|---|---|---|
| 0 | `onlyHook`：`factory != 0 && factory.registeredHooks(msg.sender)` | 否则 revert `OnlyHook` |
| 1 | `piggybackActive()` 已置位 → **静默 return** | 嵌套 Tosh 池 poke 我们时保持被动 |
| 2 | `!poolManager.isUnlocked()` → return | 必须在别人的 unlock 帧内才能碰 swap/settle/take |
| 3 | `balance < TRIGGER_STEP(1 ether)` → return | 未武装 |
| 4 | `ladderTokens.length == 0` → return | 无策展标的 |
| 5 | `count = min(total, BATCH_SIZE=3)`；`perToken = 1 ether / count` | 每轮恰好花 1 ETH |
...
| 9 | `_setPiggyback(false)`；`emit PiggybackExecuted(perToken*count, count, newCursor)` | |

**单腿 `_buyAndBurn`**（`src/ToshLadderTreasury.sol:313-352`）：

```
swap(key, { zeroForOne: true,                          // ETH(currency0) → token(currency1)
            amountSpecified: -int256(ethIn),           // 负数 = exact input
            sqrtPriceLimitX96: MIN_SQRT_PRICE + 1 })   // 无滑点下限
sync(native) ; settle{value: ethIn}()                  // 付 ETH 腿
take(currency1, DEAD_ADDRESS, bought)                  // 代币直送 0xdead
emit BuybackBurned(token, ethIn, bought)
```

**不设 `sqrtPriceLimitX96` 保护的理由**（`src/ToshLadderTreasury.sol:323-326`）：产出被销毁，不利价格只是少烧一些币。没有受害者、没有可提取的 MEV（什么都不返回给调用方），这正是 v5.0 能删掉链下 harvest 机器人的滑点底线机制的原因。

**关键行为**：`autoPiggybackBuyback` **绝不因"未就绪"而 revert**（`src/ToshLadderTreasury.sol:262-266`）——它坐在普通用户 swap 的热路径上，在这里 revert 会让池子不可交易。所有前置条件都是早返回。

> **⚠️ 8.21**：每次触发只花掉恰好 `TRIGGER_STEP = 1 ETH`，无论国库余额是 1 ETH 还是 100 ETH。超出部分只能靠后续交易逐笔泄放。
>
...
未开盘的 hook 没有 pool key（零 key），会在 `currency1` 那一臂失败（测试 `test_ladderCuration_rejectsUnlaunchedProjects` @ `test/ToshV5.t.sol:1099`，期望 `InvalidPoolKey`）。
...
`sender == ladderTreasury` 是第一层（国库自己发起的 swap 直接豁免），`_piggybackActive()` 是第二层（跨 hook 的递归）。测试 `test_piggyback_isolatesAFaultyLadderLeg` @ `test/ToshV5.t.sol:1173` 验证一条坏腿不会砸掉无辜交易者的 swap。
...
- 测试 `test_minedHookAddress_carriesV5FlagMask` @ `test/ToshV5.t.sol:314`、`test_hookMiner_requiredFlagsAre0x20CC` @ `test/ToshV5Guards.t.sol:246`。

**建池抢跑防御**：`beforeInitialize` 要求 `sender == address(this)`，否则 revert `UnauthorizedInitialization`（`src/ToshLaunchpadHook.sol:1006-1016`）。也就是说这个池子**只能**从 hook 自己的 `launch()` 里创建，外人无法抢先用别的初始价格建起同一个池。测试 `test_beforeInitialize_revertsForExternalSender` @ `test/ToshV5Guards.t.sol:489`。

### 5.4 创世流动性永久锁定 与 散户 LP 的隔离

已在 4.4 详述。补充要点：

- **锁的来源不是回调**，是"V4 按 `msg.sender` 归属仓位 + hook 无移除代码路径"。`beforeRemoveLiquidity` 现在只是个 pass-through，V4 甚至不会调它（掩码里没设那一位）（`src/ToshLaunchpadHook.sol:1018-1042`）。
- 散户 LP 走**自己的**仓位 key，加/撤都不动创世仓位。测试 `test_retailLp_canAddAndRemoveWithoutTouchingGenesis` @ `test/ToshV5.t.sol:633` 分别断言了加仓后、撤仓后创世流动性都不变。
...
`executeBuyAndBurn` 之所以是 `external`，纯粹是为了让 `autoPiggybackBuyback` 能用 `try/catch` 做逐腿故障隔离；`onlySelf` 修饰器保证除了合约自己没人能调（`src/ToshLadderTreasury.sol:306-311`、`162-165`）。测试 `test_executeBuyAndBurn_isNotCallableExternally` @ `test/ToshV5.t.sol:1229`。
...
**代码里的对应断言**（`test/ToshV5.t.sol:909-958`，测试名 `test_sweepIsProfitableOnceTheMarketHasRunAhead`）：
...
测试用 `FreeRider` 合约（持有并花自己的 ETH，避免 `vm.prank` 造成的收支不对账，见 `test/ToshV5.t.sol:1248-1257`）实际跑通了这个套利，断言 `address(rider).balance > before`，并用 `assertLt(profit, cost * 2)` 把量级钉住（防止未来的定价改动**悄悄扩大**这个窗口）。
...
> **这三个具体数字在代码中未找到依据。** 仓库里只有定性断言（`assertGt(balance, before)`）和量级边界（`profit < cost × 2`）；`test_sweepIsProfitableOnceTheMarketHasRunAhead` 用的是 `_openLadder(hook, 0.5 ether)` 这个 0.5 ETH 拉盘规模（`test/ToshV5.t.sol:940`），与上表的实验设置一致，但精确的成本/利润数值是一次测量运行的结果，不是签入仓库的断言。如需回归保护，建议把这三个数字（或它们的比值下限）固化成断言。
...
**对比：什么时候扫货是亏钱的**（`test_sweepAndDumpIsLossMaking` @ `test/ToshV5.t.sol:873-907`）：当市场**没有**跑在阶梯前面时，扫货必亏。理由是结构性的：货架铸造不触碰池子，所以拖不动 spot 跟上来；买家至少按 1.05× 市价付款，然后必须用自己的规模把同一个市场**往下砸**才能卖出，这还没算 1.00% 的往返摩擦。断言是"亏损必须 > 成本的 1/20"（实质性亏损，不是边际亏损）。
...
**被关掉的攻击路径**（注释原文，`src/ToshLadderTreasury.sol:182-196`）：早期版本允许 owner 连同代币一起传入任意 `PoolKey`，只校验币种排序。这**静默地击穿了单向阀**：owner 可以铸一个一文不值的 ERC-20，把它配在一个只有自己提供流动性的无 hook 池里，挂上阶梯，然后让每一次 1 ETH 回购都结算进自己的仓位——**一次触发抽走一点**，而链上只会看到普普通通的 `BuybackBurned` 事件。
...
测试：`test_ladderCuration_rejectsForeignTokens` @ `test/ToshV5.t.sol:1089`；`test_ladderTreasury_ownerCannotRedirectSpendToOwnPool` @ `test/ToshV5.t.sol:1216`（这个测试的 natspec 特意说明"探测有没有 `withdraw` 选择器证明不了什么，真正的抽资路线是重定向支出方向"）。
...
| 可选 kill-switch | `renounceMinterRole()` 保留为紧急/治理逃生口，**不会自动触发**；一旦调用不可逆 | `src/ToshToken.sol:116-127` |
...
| 创作者绑定盐 | `finalSalt = keccak256(abi.encode(creator, rawSalt))`，别人的盐在你身上无效 | `src/ToshFactory.sol:367` |
| `SafeCast.toInt128(tax)` | 交回 V4 flash accounting 的唯一数值做了检查转换，静默截断会错报抽税额 | `src/ToshLaunchpadHook.sol:1118-1120` |
| TWAP 向负无穷取整 | 对齐 Uniswap V3 `OracleLibrary`，保证 TWAP 不被截断上偏 | `src/ToshLaunchpadHook.sol:1320-1322` |
...
| `soat-frontend/src/app/projects/page.tsx` + `ProjectsRadarClient.tsx` | 项目列表 | 目录/雷达视图 |
| `soat-frontend/src/app/projects/[address]/page.tsx` | 项目详情 | 装载 `ProjectTerminal` |
| `soat-frontend/src/components/ProjectTerminal.tsx` | **项目终端**（规范实现，73KB） | 相位状态机 + 创世/阶梯/LP/退款四个面板 + 事件流 |
| `soat-frontend/src/app/components/ProjectTerminal.tsx` | 兼容 shim | 只有一行 `export { default } from '@/components/ProjectTerminal'`（`:18`） |
| `soat-frontend/src/app/admin/page.tsx` | Owner Command Center（51KB） | 软顶/发射费/额度/冷却/黑名单/签名者，全部走工厂的 owner 函数 |
| `soat-frontend/src/app/components/UserDrawer.tsx` | 个人主权控制台（33KB） | PoG 额度、冷却矩阵、已参与资产 + `claimGenesis` 入口（`:655`） |
| `soat-frontend/src/app/components/NetworkGuard.tsx` | 网络守卫 | 见 6.3 |
...
`soat-frontend/src/app/components/NetworkGuard.tsx:12-42`：
...
- 所有写操作都显式带 `chainId: BASE_SEPOLIA_ID`（例如 `useTosh.ts:95`、`ProjectTerminal.tsx:747`、`1053`），让 wagmi 在错链时直接拒绝而不是发到错的链上。
...
**没有做的失效**：`defaultSoftCap` 或 `maxPogAllocationLimit` 被 owner 在"挖盐"与"提交"之间改动。这种情况下 `createLaunch` 会 revert `InvalidHookSalt`——测试 `test_createLaunch_revertsWhenSoftCapRotatedAfterMining` @ `test/ToshV5Factory.t.sol:558` 固化了这个行为。前端没有重试提示，用户只能重新挖盐。
...
`soat-frontend/src/components/ProjectTerminal.tsx:1574-1593`，单次 `useReadContracts` 拉 14 项（12s 轮询）：

`totalEthDeposited`、`launched`、`p0`、`phase2Minted`、`canRefund`、`genesisDeadline`、`softCap`、`BONDING_MAX`、`currentBondingPrice`、`ethDeposited(user)`、`factory.pogQuota(user)`、`factory.totalGenesisDeposited(user)`、`factory.userLaunchCooldownEnd(user, hook)`、`shelfP0`。

`projectToken` 单独读且 `staleTime: Infinity`——部署即固定，12s 轮询是浪费（`:1615-1620`）。

类型显式声明为 `ContractFunctionParameters[]` 而非交给推断：HOOK_ABI 约 130 条，wagmi 的逐项映射类型会炸掉 TypeScript 的实例化深度上限（`:1567-1573`）。

#### 6.6.2 相位状态机

```ts
// ProjectTerminal.tsx:669-680
...
writeDeposit({
  ...
  args: [p.hookAddress, ZERO_ADDRESS],   // ← 推荐人硬编码为零地址
  value: amountWei,
  chainId: BASE_SEPOLIA_ID,
})
```

> **⚠️ 8.3（影响面最大的一条）**：**推荐人被硬编码为 `ZERO_ADDRESS`。** 合约侧的全局终身推荐图（`globalReferrers`、`referralCount`、`ReferralBound` 事件、`referrerOf` view）完整齐备，ABI 里 `claimReferralReward` / `claimableReferral` / `referralAccrued` 全都在，但：
> - 前端**没有任何** `?ref=` / `searchParams` 解析（全仓搜索 `searchParams` 在 `soat-frontend/src` 里零命中）；
> - 也没有推荐人输入框；
> - `useContractActions.genesisDeposit` 的 `referrer` 参数默认值同样是零地址（`soat-frontend/src/app/lib/useContractActions.ts:55`）。
>
> **实际后果**：经官方 UI 完成的每一笔创世出资，10% 都会走 `orphanReferral` 路径最终进平台国库。**推荐佣金在产品上目前不可获得**，除非用户绕过 UI 直接构造交易。这是一条完整实现的合约功能被前端完全旁路的情况。
>
> **⚠️ 8.4**：与之配套，`claimReferralReward()` **在整个前端没有任何调用入口**（只出现在 ABI JSON 里）。
...
- 四个读数：ACTIVE SHELF `#i / 2000`、SHELF PRICE、REMAINING、105% CEILING（解锁时用 fluo 色）。
- 当前档填充进度条。
- 5 行档位表：`#索引 / 价格 / 已售% / LIVE|CLEARED|QUEUED`。
- 底栏：`P₀ = … / spot = … / twap = …`（把三个价格摆在一起，让用户自己看门控逻辑）。

**四个读数栏**（`:1084-1097`）：`P₀ · POOL OPEN`、`SHELF 0 · +5%`（提示 "mint premium over market"）、`ACTIVE SHELF`（提示 `X× ladder base`）、`PHASE-2 MINTED`（`phase2Minted / bondingMax`，提示 `2000 shelves × 8.4K`）。
...
- 两条性质都由 `scripts/checkLpActions.ts` 对着真实解码器的规则做断言。
...
日志扫描失败时设 `degraded = true`，面板显示降级提示（`ProjectTerminal.tsx:1444-1450`）："this RPC would not serve position logs, so only positions minted from this browser are listed. Your other positions are safe on-chain and remain withdrawable through any Uniswap V4 interface."
...
| 环节 | 位置 | 说明 |
|---|---|---|
| 会话授权签名 | `ProjectTerminal.tsx:99-126`、`buildPoGScanAuthMessage`（`contracts.ts:140-142`） | 消息格式 `"Tosh PoG Scan Request\nAddress: {addr}\nTimestamp: {ms}"`，签名缓存在 `sessionStorage`，TTL `POG_SESSION_AUTH_TTL_MS = 1,800,000`（30 分钟） |
...
| 常量 | 值 | 行号 |
|---|---|---|
| `TRIGGER_STEP` | 1 ether | `ToshLadderTreasury.sol:70` |
| `BATCH_SIZE` | 3 | `ToshLadderTreasury.sol:75` |
| `_PIGGYBACK_SLOT` | `0x546f73685069676779626163b1000001` | `ToshLadderTreasury.sol:85` |
...
| `SpanTooManyShelves` | 跨档 > 16 | "拆成两笔发送，终态相同" |
...
#### Treasury（`src/ToshLadderTreasury.sol:127-135`）

`OnlyHook` / `OnlySelf` / `FactoryAlreadySet` / `FactoryNotSet` / `ZeroAddress` / `TokenAlreadyListed` / `TokenNotListed` / `TokenNotLaunchedHere` / `InvalidPoolKey`
...
| 需求 | 测试 | 位置 |
|---|---|---|
| 10% 创世溢价（关系而非硬编码） | `test_genesisPremium_isExactlyTenPercent` | `test/ToshV5.t.sol:595` |
...
| 前端字节码同步 | `test_hookBytecode_inSyncWithArtifact` | `test/ToshV5Bytecode.t.sol:17` |
...
本章第 8.1–8.15 系列已无未决项。红队追加的 8.26–8.31 中，8.29–8.31 保留了**需要产品决策的开放项**，见 §11。
...
- **⚠️ 行号锚点已系统性失效。** 本文大量使用 `src/ToshLaunchpadHook.sol:857-877` 这类锚点。红队那轮往 Hook / Factory / Treasury 里插入了数十行说明性 natspec，所有位于插入点之后的锚点都已偏移。§1.2、§2.2.8、§5.1、§8.26–8.31 中被触及的锚点已改为**函数名**，其余未逐一校准。
