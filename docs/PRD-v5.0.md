...
**Event stream: nothing in the app has one.** The project terminal never did, and the directory home's was removed by the v0 home-page redesign. The app now subscribes to no contract event anywhere, so every live figure on every screen is an HTTP poll on a `refetchInterval`.

The project-terminal half has always read the same way: this section originally described a `RecentEventsTicker` subscribing to `Deposited` / `TierMinted` / `Refunded` with basescan links. No such component ever existed, and the explorer detail was wrong too — this chain's explorer is Blockscout.

The directory home's feed **did** exist, and this paragraph described it accurately, so here is what was removed rather than a claim it was never there. `TxFeedMarquee` @ `soat-frontend/src/components/TxFeedMarquee.tsx`, mounted through `EventTickerStrip` @ `soat-frontend/src/components/directory/EventTickerStrip.tsx` with `ssr: false` because it ran on `useWatchContractEvent`; the wrapper was called `A2AFeed` until commit `50bc9a7` renamed it. It subscribed to three **factory** events — `LaunchCreated` / `PoGRegistered` / `GenesisDeposit` — held a `RING_LIMIT` 24-entry ring buffer, deduplicated then prepended and trimmed, and rendered the strip twice head-to-tail for seamless scrolling, pausing on hover. It carried no explorer links. All of that is recoverable from `50bc9a7` if it is ever wanted back.

The removal is one coherent change across three files: both components deleted, and `AgentDirectoryHome.tsx` stripped of the `import` and the `<EventTickerStrip />` mount. That space now goes to `HeroFeedPanel` @ `soat-frontend/src/components/directory/HeroFeedPanel.tsx`, which lists the newest **launches** from `useDirectoryProjects` — one row per project on a poll, not one row per event.

For whoever rebuilds a ticker: the dependency the old one carried is worth knowing before starting. `useWatchContractEvent` over the default HTTP transport degrades to a `getLogs` poll, because no environment here configures a WebSocket transport and whether `rpc.*.chain.robinhood.com` serves `wss://` has never been verified. The `ssr: false` on the wrapper was about the subscription, not about SSR being hard.

**A note on how the removal was detected, and on an invalid inference made while auditing it.** `checkDocSymbols.mjs` went red on `TxFeedMarquee`, `EventTickerStrip` and `A2AFeed`. It reads the files `git ls-files` reports and skips ones it cannot open, so while the deletion was still uncommitted the three tracked-but-absent files read to it as symbols existing nowhere in the tree — which is the guard doing its job, because an uncommitted deletion of a live feature is exactly the drift it is for. It was briefly misread as documentation drift instead, and this paragraph was rewritten to assert the three components had never been written, on the reasoning that `git log --diff-filter=D` found no delete commit for them. **That inference is invalid**, and it is recorded here rather than quietly fixed: a deletion that has not been committed yet produces no delete commit either, so the absence of one proves nothing about whether a file ever existed. `git ls-files` and `git show HEAD:<path>` are what answer that question.
