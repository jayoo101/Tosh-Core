...
| **PM-E5** | First incident drill run and dated | `INCIDENT_RESPONSE.md` §8 drill log | 🟡 **Two of Q1's three criteria now met.** §8.1 (2026-09-03) did the on-chain half through the deployer EOA. §8.2 (2026-09-04) redid it through a real **2-of-3 Safe** that held ownership — `pause()` 107,816 gas with two signatures, mechanical path 5 s, window 6 min 31 s — and carried the announcement on the public status page **both ways**, which is the criterion §8.1 explicitly left open ("does not become a pass until a drill actually posts to the page and back"). Page propagation measured at 8.9 / 21.3 / 32.0 s. Controls confirmed the brake really had moved: the deployer's own `pause()` reverted `OwnableUnauthorizedAccount`, and so did a lone signer's. Q1's **third** criterion — "at least one *new* signer participating" — is still unmet, but is **no longer blocked on D4**, which closed. §8.3 (2026-09-04) built the machinery for it: a 2-of-3 drill Safe on 46630 `0x853D416A…` owned by the **real** signer set with the **deployer deliberately not an owner**, so nothing in the drill can move without two signatures from people who are not the operator — the asymmetry neither §8.1 nor §8.2 could test. The four payloads are staged and published for off-chain EIP-712 signing, all four hashes verified against the Safe's own `getTransactionHash()` (guarded in CI, 10 mutations caught), and the signature shape proven accepted with a single signature proven refused `GS020`. What remains is **two people signing** — which §8.2 measured as almost the entire budget — plus Twitter/Discord/`#status` account signups |
...
| **PM-D4** | 🟡 | Safe created 2026-09-04: `0x2953957774482efA660921df85A1E7634ccfe27A`, 2-of-3 SafeL2 1.4.1 on 4663, indexed by the transaction service, all three owners signature-verified, `verifyOwnerSafe.mjs` green. This unblocks **C1**, which takes `PROD_OWNER_SAFE` as a required input, and supplies PM-C9's `PLATFORM_TREASURY`. Remaining: `INCIDENT_RESPONSE.md` §1 still lists the owners as addresses rather than reachable people, and Q1's third criterion wants one of the new signers in a testnet drill. |
...
| **PM-E5** | 🟡 | Two of Q1's three criteria met as of §8.2 (2026-09-04): pause/unpause through a 2-of-3 Safe inside the window, and the status page carrying the announcement both ways. The third — a *new* signer participating — is staged and verified in §8.3 against a drill Safe the operator does not own, and now waits only on two of Tom / Jack / Joe signing. Three of Step 4's four comms channels remain account signups. |
