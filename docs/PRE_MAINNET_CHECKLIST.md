...
| **PM-C4** | Contracts verified on the block explorer | Public verified source at the deployed address | 🟡 **two of three verified, and the third is one this row never knew about.** `0xBa9d2E86…` and `0x99aD248d…` both read **"Contract source code verified (exact match)"** as of 2026-09-09 — `ToshFactory` at 23:18:07 and `ToshLadderTreasury` at 23:26:10, `v0.8.26+commit.8a97fa7a`, cancun, optimizer on / 200 runs, MIT, constructor arguments decoded, Read/Write tabs live. Not partial matches. **Still unverified: `HookDeployLib` at `0x873E0841bc0d8F87102E2a2862a0d32D1b890462`**, a third deployed contract whose address appears nowhere outside `run-latest.json`'s `libraries` field. `forge script` deploys libraries through the canonical CREATE2 proxy `0x4e59b448…`, so it is not a `CREATE` entry, the deployer's nonce sequence (6–10) skips it, and every count of "what we put on mainnet" said two. It is **not** a live surface — both call sites are in the factory's constructor and the deployed factory's runtime code contains zero references to the address — so this is an audit-trail gap, not a safety one, and it is why this row is 🟡 rather than ❌. Inputs for all three: `node scripts/genVerifyInput.mjs`. **`--verify` in the broadcast never could have worked**: the whole `/api` surface is behind a Cloudflare *managed challenge*, so forge gets an HTML interstitial where it expects JSON and reports `Failed to deserialize response: expected value at line 1 column 1` — a parse error, not an access error, scrolling past under the deployment output. The HTML surface is unchallenged, so the browser form is the only route, and no CI runner can check this either. Verification was possible at all only because `src/`, `foundry.toml` and the `lib/` pointers are byte-identical to deploy commit `d0220e2`. See `SECURITY_AUDIT.md` §5.33 |
...
material; it is `0x73db078f…`, which the same file records as its
`POG_SIGNER_ADDRESS` — so this was the Base-era PoG signer, doing double duty
as deployer. Every address beside it is Base, not Robinhood: the RPCs are
`sepolia.base.org` and `mainnet.base.org`, and the factory, treasury and
`HOOK_DEPLOY_LIB` it names are none of the 4663 contracts. The file's name is
accurate — it predates the chain migration entirely.

Exposure, measured rather than assumed, because the file carries a
`BASE_MAINNET_RPC` and that is the line that would make it matter: on Base
mainnet the address has **nonce 0 and zero balance** — the key has never sent a
transaction there and holds nothing. On Base Sepolia it has nonce 46 and
0.048593 testnet ETH. So the residue is a faucet-funded testnet key with no
real-money footprint on any live chain, which is the disposition the name
implied and nobody had verified.
...
So even moved into scope, this file would still pass — and the bare name is not
hypothetical, it is the Foundry convention this repository used for the
deployer, the same `PRIVATE_KEY` that §5.31 records destroying from
`.env.production`. A deployer key dropped into `soat-frontend/.env.local` under
that name is invisible to this guard today. That is the finding; §5.31's
"the check never looked for a copy" has a sibling in "the check does not know
the name the copy would use".
...
| **PM-C4** | 🟡 | Factory and treasury verified 2026-09-09, exact match. Remaining is `HookDeployLib` `0x873E0841…` — a third mainnet contract, CREATE2-deployed by `forge script` and absent from every previous count. Not a live surface (constructor-only; no runtime reference), so audit trail rather than safety. Inputs: `node scripts/genVerifyInput.mjs`. |
...
is verification (C4's `HookDeployLib`), the announcement (C3), and the first
ladder listing (C8). **Corrected 2026-09-09.** This paragraph gated the last
...
`check:secrets` is 27/27 green. The sentence that stood here — "that is not a
