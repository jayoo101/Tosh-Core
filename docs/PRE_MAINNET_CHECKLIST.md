...
| **PM-D4** | ❌ | Gnosis Safe, 2-of-3, three reachable signers. Blocks **C1** — `DeployMainnet.s.sol` takes `PROD_OWNER_SAFE` as a required input — as well as C2 and the human half of E5. Tooling verified on both chains and driven through the real `onlyOwner` path with two signatures (§8.2). Two candidates found 2026-09-04; the remaining work is scripted in `verifySignerCandidates.mjs` → `createOwnerSafe.mjs` → `verifyOwnerSafe.mjs`. |
