...
| ID | Item | Evidence of done | Status |
|---|---|---|---|
| **PM-D1** | PoG signer is a **new** key, distinct from the deployer, held only in the production secret store | Cutover: `factory.pogSigner()` ≠ deployer; `POG_SIGNER_PRIVATE_KEY` set in Vercel Production only; absent from every laptop `.env*`. See §4.1 | 🟡 on-chain half is done: `factory.pogSigner()` is `0x0E496Bd5…`, not the deployer. The custody half was **known violated** — the live key reached PSReadLine history in plaintext (`SECURITY_AUDIT.md` §5.32). The 2026-09-08 disposition was accepted-deviation; it was **reversed 2026-09-09 and rotation is in progress**, because lifting PM-C3 and PM-C8 while keeping the key is the conversion event the gate exists to prevent, not a second independent decision. Goes green when all five parts land, not on the strength of the on-chain half: new key in Vercel Production, frontend redeploy, `setPogSigner` from the Safe, `POG_PRIVATE_KEY` still unset (now inventoried `absent` — it is a second legal name for the same key), and `MONITOR_EXPECTED_POG_SIGNER` repointed, since `STATE-04` compares it against `factory.pogSigner()` and it still holds the leaked address |
...
in one MultiSend batch. Built and verified 2026-09-08; both owners have
signed `safeTxHash` `0xffe2da61…` as of 2026-09-09, and it is **still not
executed** — Safe `nonce()` is 1 and both orphans read `owner()` =
`0x4e41cea9…` with `pendingOwner()` = the Safe, which is the pre-batch
state exactly. See `SECURITY_AUDIT.md` §5.31, and §5.26 for the orphan
pause.
...
| **PM-D1** | 🟡 | On-chain half done. Custody half violated: the live key reached PowerShell history in plaintext (`SECURITY_AUDIT.md` §5.32). Accepted-deviation reversed 2026-09-09; rotation in progress, five parts, two of them off-chain (the `POG_PRIVATE_KEY` alias and `MONITOR_EXPECTED_POG_SIGNER`). Still the gate on PM-C3 and PM-C8. |
