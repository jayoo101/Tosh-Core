...
| ID | Item | Evidence of done | Status |
|---|---|---|---|
| **PM-C1** | `DeployMainnet.s.sol` run against the production RPC | `broadcast/DeployMainnet.s.sol/4663/run-latest.json`; factory `0x2920ca7E9fcD85491D699e1f9Ae2CAa65Cfb2892`, treasury `0x255722226720914eF5B2CD54647f21f584BD4Ea2`; block 61056709 | ✅ **run twice. Current: 2026-09-12, chain 4663, block 61056709**, from commit `9b9d9ce` (`d18d2d5` on `main`), because the 8 % / 2 % referral split is `immutable` and could not be changed in place. Same script, same roles, same Safe; `C1_RUNBOOK.md` §0-bis lists what carried over, what had to be redone, and the two costs (every user's PoG attestation resets with the factory, and the two 2026-09-08 test projects stay on the old factory unmigrated and unlisted). The earlier run, whose record the rest of this row is: **2026-09-08, chain 4663**, factory `0xBa9d2E86281b988225Eca383C375215912fb20B9`, treasury `0x99aD248dD15498957B864Fd79917F0E103Aa78F7`, blocks 57400516–57400521 — now out of scope, still on chain, not broken. Five transactions, all `status=0x1`. 9,353,658 gas, 0.0026585890501 ETH. Mutually wired. Ownership was staged in this broadcast (`owner()` the deployer, `pendingOwner()` the Safe) and has since been accepted — see PM-C2. The 46630 rehearsal (RH-F1) remains the behavioural proof — launch, claim, buy, shelf mint, TWAP-matured `addLadderToken`. See the note below and `C1_RUNBOOK.md` §0 |
| **PM-C2** | Gnosis Safe has called `acceptOwnership()` on **both** factory and ladder treasury | `VerifyDeployment.s.sol` passes with `EXPECTED_OWNER=<safe>` | ✅ **re-done 2026-09-12 on the redeployed pair, and it had to be**: `acceptOwnership()` is per-contract, so the 2026-09-08 acceptance said nothing about the new addresses and the new pair spent a window owned by the deployer exactly as the old one had. Factory `0x2920ca7E…` and treasury `0x255722…` now both read `owner()` = Safe `0x2953957774482efA660921df85A1E7634ccfe27A` and `pendingOwner()` = zero, and `VerifyDeployment.s.sol` passes against them with `EXPECTED_OWNER`, `EXPECTED_POG_SIGNER` and `EXPECTED_PLATFORM_TREASURY` set. The single-key window is closed again. The 2026-09-08 acceptance, which the rest of this row records, remains true of the old pair: ✅ **chain 4663, block 57455937**, tx `0x002ad51544aa6b7377d689bf30f4822e45278a882887bf1fa6f363a95ed4b3eb`. Single batched Safe transaction via MultiSendCallOnly v1.4.1 at `0x9641d764fc13c8B624c04430C7356C1C7C8102e2`, operation = 1 (DELEGATECALL), Safe nonce 0. `safeTxHash` `0x13602041beeb67d02fb828c79502839a0f2a65a663c43d1d0646bd4c8ec17ea1`. Status success, gasUsed 106151, submitted by signer `0xC2EA14cE2112B18AFBC78fE78C969b3002F07cbB`. Result: `ToshFactory` `0xBa9d2E86281b988225Eca383C375215912fb20B9` and `ToshLadderTreasury` `0x99aD248dD15498957B864Fd79917F0E103Aa78F7` both now have `owner()` = Safe `0x2953957774482efA660921df85A1E7634ccfe27A` and `pendingOwner()` = zero. Safe nonce is now 1. `VerifyDeployment.s.sol` passed with `EXPECTED_OWNER`, `EXPECTED_POG_SIGNER` and `EXPECTED_PLATFORM_TREASURY` all set. The single-key window is closed. Mechanism was rehearsed on 46630, 2026-09-04 (`INCIDENT_RESPONSE.md` §8.2). See `SECURITY_AUDIT.md` §5.26; the UI-path inference in that sweep is corrected in §5.29 |
...
| **PM-C4** | Contracts verified on the block explorer | Public verified source at the deployed address | ✅ **re-closed 2026-09-12 on the redeployed contracts, and this time it is five contracts at `partial match` rather than three at exact.** `ToshFactory` `0x2920ca7E…`, `ToshLadderTreasury` `0x255722…`, the `ToshLaunchpadHook` `0xa90CF811…` and `ToshToken` `0xc4184708…` implementations the factory builds in its constructor, and `HookDeployLib` `0x6a02d980…`. Confirmed against `is_verified` on `/api/v2/addresses/<addr>`, not against the page — see the two traps at the end of this row. `partial match` here means runtime bytecode byte-identical and the trailing metadata hash not, and `is_verified_via_sourcify` is `false` on all five, which is accurate: Blockscout compiled the standard-JSON input itself rather than importing the Sourcify records, so the two verifications are independent and agreeing. All five also on Sourcify as `match`. Route and inputs: `verify-out/README.md`, `node scripts/genVerifyInput.mjs`. **The 2026-09-08 record below stands as history and differs on both counts — three contracts, exact matches:** all three verified, and the third was one this row never knew about. `ToshFactory` `0xBa9d2E86…` 2026-09-09 23:18:07 and `ToshLadderTreasury` `0x99aD248d…` 23:26:10, both **"Contract source code verified (exact match)"**, constructor arguments decoded, Read/Write tabs live. `HookDeployLib` `0x873E0841bc0d8F87102E2a2862a0d32D1b890462` **2026-09-11 02:23:12Z**: `is_fully_verified` true, `is_partially_verified` false, `is_changed_bytecode` false — a full match, not a metadata-stripped one — declared as `src/libraries/HookDeployLib.sol` over 1 + 44 sources. All three `v0.8.26+commit.8a97fa7a`, cancun, optimizer on / 200 runs, `viaIR` true, MIT. **That third contract is why this row sat at 🟡 for two days**: its address appears nowhere outside `run-latest.json`'s `libraries` field, because `forge script` deploys libraries through the canonical CREATE2 proxy `0x4e59b448…`, so it is not a `CREATE` entry, the deployer's nonce sequence (6–10) skips it, and every count of "what we put on mainnet" said two. It is **not** a live surface — both call sites are in the factory's constructor and the deployed factory's runtime code contains zero references to the address — so this was an audit-trail gap, not a safety one. A full match struck three days after deployment is itself the reproducibility evidence: it holds only because `src/`, `foundry.toml` and the `lib/` pointers are byte-identical to deploy commit `d0220e2`. Inputs for all three: `node scripts/genVerifyInput.mjs`. **`--verify` in the broadcast never could have worked**: the whole `/api` surface is behind a Cloudflare *managed challenge*, so forge gets an HTML interstitial where it expects JSON and reports `Failed to deserialize response: expected value at line 1 column 1` — a parse error, not an access error, scrolling past under the deployment output. The HTML surface is unchallenged, so the browser form is the only route, and no CI runner can check this either. See `SECURITY_AUDIT.md` §5.33. **Two things the 2026-09-12 round found that this row did not know, and both are reasons to distrust the page rather than the API.** First, Cloudflare was not the last obstacle: driven from inside a real browser the POST goes through and the endpoint answers `429 {"message":"Too many requests"}` with `x-ratelimit-limit: 1` and `x-ratelimit-reset` around 18 minutes, so it is **one verification per ~18-minute window per client** and five contracts is five windows. Nothing in the page says so — the form re-enables itself with the file still attached, which is indistinguishable from a rejected payload, and a submission made early spends the attempt and appears to restart the window, so the correct move after a 429 is to leave the page alone. Second, the method Blockscout's own form calls **`sourcify` is not a way to verify on Blockscout at all**: it renders an embedded `verify.sourcify.dev` widget that submits to Sourcify and shows a green "Match" badge reporting *Sourcify's* status, so it reports success for a contract Blockscout still lists as unverified. Two attempts were fooled by it before anyone checked `is_verified`. The method that works is **Solidity (Standard JSON input)** |
| **PM-C5** | `forge build --sizes` — every contract under the 24 KB EIP-170 limit | Build output | ✅ see §3.1 |
| **PM-C6** *(legacy `#23`)* | Hook initcode hash regenerated against the **mainnet** build | `RecomputeInitcodeHash.s.sol` output committed; `extractAbis.js` produces no diff | ✅ **re-measured 2026-09-12 against the new factory `0x2920ca7E…`, and the number moved as it had to.** `HOOK_CREATION_CODEHASH` is now `0xece3b0c259085201dda4d6a2ab94ed469a1d806b498a4d8c411b74ecd0b549f4` and still equals `keccak256(type(ToshLaunchpadHook).creationCode)` of this tree; the script reverts `HookCreationCodehashMismatch` otherwise, so a clean exit is the assertion. **Do not read the change as drift.** The referral split changed the hook's creation code, which is the entire reason there was a redeploy, so a codehash equal to the old one would have meant the new bytecode had not shipped. `.env.production` refilled with this value and `DEPLOY_BLOCK=61056709`. Prior measurement, which the rest of this row records: ✅ **2026-09-08, chain 4663, block 57592077**, `RecomputeInitcodeHash.s.sol` run against factory `0xBa9d2E86281b988225Eca383C375215912fb20B9` with `--sig 'run(address)'`. `HOOK_CREATION_CODEHASH` `0xc43a20c91d0f3164cdeb07d8786c61184c105825a9edec30a8df949f41b4d139` matches `keccak256(type(ToshLaunchpadHook).creationCode)` of this tree; `getLiveHookInitcodeHash()` `0x3a706af1817f0f630ccde8389a67d0bffd6a4744f5e4e0dc6e914bb8bd0e91ef` (clone initcode; not comparable to the former). Output committed in `SECURITY_AUDIT.md` §5.30. `extractAbis.js` produced no diff. `.env.production` still holds the `0x` placeholders — that file is gitignored and is operator work, not this row's evidence. The launch page reads `factory.hookInitcodeHash(...)` from chain either way. See `SECURITY_AUDIT.md` §5.30 |
| **PM-C7** | `.env.production` filled: `NEXT_PUBLIC_FACTORY_ADDRESS`, `NEXT_PUBLIC_CHAIN_ID` — **and the status page's `CHAIN` block repointed** | Deployed frontend reads the right factory; `checkStatusPage.mjs` green | ✅ **both halves closed for the current deployment, and both had to be closed a second time.** The redeploy broke each half in its own way, which is the argument for this row having two halves at all. Frontend: `NEXT_PUBLIC_FACTORY_ADDRESS` repointed to `0x2920ca7E…` and shipped in PR #31; confirmed 2026-09-12 by reading the deployed chunks on `toshx.xyz`, where the new factory and treasury appear and the old factory appears in **none** — a negative as well as a positive, because a half-cutover is what this row exists to catch. Status page: it was still reading `paused()` off the retired `0xBa9d2E86…` **four days after the redeploy**, found 2026-09-12 and fixed in `jayoo101/tosh-status` at `8099867`. That staleness was worse than cosmetic: the page's whole function is to say whether the live factory is paused, so pausing the real one during an incident would have left it reporting operational, and its two explorer links pointed users at a contract nobody uses. **Nothing catches this** — the page is in another repository, hardcodes its own `CHAIN` block, and `checkStatusPage.mjs` checks that the page is reachable and coherent rather than that its factory is the one this repo deploys. Prior record: ✅ **both halves closed 2026-09-04 / 2026-09-09**, when the status page named Robinhood mainnet 4663, factory `0xBa9d2E86…`, explorer `robinhoodchain.blockscout.com`. That half was added 2026-09-04 because C7 had never covered it — the page hardcodes its own chain in a different repository (`SECURITY_AUDIT.md` §5.27). Frontend: the first production deploy reported ready carried the mainnet factory and treasury with `NEXT_PUBLIC_CHAIN_ID` still `46630`, so the live site pointed a testnet connection at contracts that have no code on 46630 and rendered nothing. It was found by reading address literals out of a minified chunk by hand, which is why `checkDeployedChain.mjs` now exists. Verified 2026-09-09 against `https://tosh-two.vercel.app`: 19 chunks scanned, badge reads mainnet, chain id **4663**, both addresses resolve to the 4663 deployments, `coherent`. Re-check with `npm run check:deployed -- --url …`, never by eye |
| **PM-C8** | Ladder buyback targets curated (`treasury.addLadderToken`) — **no token listed until its TWAP has matured**, see §3.2 | On-chain state; `STATE-07` green | ❌ for mainnet, **no longer gated**. The PoG rotation that blocked this landed 2026-09-09 (`SECURITY_AUDIT.md` §5.32), so a listed token no longer hands value to a leaked key. No token has been listed on either 4663 treasury, and the 2026-09-12 redeploy reset the seven preconditions below rather than carrying them: they were measured on hook `0xb0AfF0c8…`, which belongs to a project on the **old** factory, so a listing on the new treasury `0x255722…` needs a project launched through the new factory and its own TWAP maturity poll. Nothing here is invalidated — the procedure and §3.2's rule are unchanged — but the measurements are of a token that the current treasury cannot list. Procedure rehearsed correctly on 46630 (`ROBINHOOD_MIGRATION.md` §F.8), which is also where §3.2's "poll, don't compute" caveat came from — the first testnet sitting had listed 52 s after launch. **Preconditions measured on 4663 on 2026-09-11 and all seven hold**, so what is left is a signature and not an investigation: §3.2's rule is satisfied (`twapSqrtPriceX96()` on hook `0xb0AfF0c8…` is non-zero, i.e. matured), the token is not already listed, `treasury.factory()` is the canonical factory, `factory.tokenToHook()` returns that hook, `launched()` is true, and `getPoolKey()` is `(0x0, 0x88382f16…, 3000, 200, 0xb0AfF0c8…)` — ETH as `currency0` and the token as `currency1`, which is the arm that would otherwise send `_buyAndBurn` the wrong way round. Sender must be the owner Safe; calldata is `0x896d5bf2` + the token word |
...
> **PM-C1 ran on 2026-09-08, and again on 2026-09-12.** The table below is the
> **first** run and its addresses are retired; `run-latest.json` now holds the
> second. Current: factory `0x2920ca7E9fcD85491D699e1f9Ae2CAa65Cfb2892`, treasury
> `0x255722226720914eF5B2CD54647f21f584BD4Ea2`, block 61056709, commit `9b9d9ce`.
> The gas and per-transaction detail below is kept because it is the only place it
> is written down, and because the two runs are close enough that it remains the
> useful estimate for a third.
>
> Chain 4663, blocks 57400516–57400521, five
...
**The condition this item set for its own retirement has since been met.** What
it said was that the fix could not reach the then-live treasury
`0x99aD248dD15498957B864Fd79917F0E103Aa78F7` — true, and for the reason given,
`ToshFactory.ladderTreasury` being `immutable` and baked into the hook
implementation every launch clones — and that the item could be retired only for
a platform deployed from source at or after that date. The 2026-09-12 redeploy is
exactly that. The live treasury is now `0x255722226720914eF5B2CD54647f21f584BD4Ea2`,
built from `deploy-4663-2026-09-12`, whose `ToshLadderTreasury.sol` carries the
gate at lines 390–394 in a reachable path: a `try` on `twapSqrtPriceX96()` that
reverts `TwapNotMature` both when the call answers zero and when it reverts.

**The item is nonetheless left standing here, and now for a different reason than
before.** The gate is on chain, so §3.2's rule is no longer the whole control —
but the procedure costs one read-only script run, `preflightLadderListing.mjs`
answers seven questions the gate does not, and no token has yet been listed on
the new treasury, so nothing has exercised the gate in production. Retire it when
a listing has actually been refused by the chain, not before.

**Do not try to confirm the gate by searching the runtime bytecode for the
`TwapNotMature` selector `0x6b0cd1ba`.** That was tried and it comes back absent,
which is not evidence of anything: under `viaIR` this contract's custom-error
selectors are often materialised rather than stored, so only **six of the fourteen
errors `ToshLadderTreasury` declares** leave a literal selector in the deployed
runtime. `InvalidPoolKey` `0xc256622b` and `OnlyHook` `0x5a91834f` are findable;
`TokenAlreadyListed` `0xdeaabdc2`, `FactoryNotSet` `0xa7df7fac`, `TokenNotListed`
`0xc5f0a1ee` and `TwapNotMature` `0x6b0cd1ba` are not, and every one of those is
declared and reachable. A selector search is a test that fails for compiled-away reasons and
will make a correct deployment look wrong. The evidence that holds is the
Blockscout verification: the runtime bytecode at `0x255722…` matches a
compilation of source containing the gate, so the gate is in what is running.
Probing it live is not available either — `addLadderToken` is owner-only, so any
call from a non-Safe address reverts `OwnableUnauthorizedAccount` `0x118cdaa7`
long before reaching the TWAP check.
...
| **PM-E2** | 🟡 | Re-pointed at 4663. First mainnet pass was blind (RPC 429, non-paging WATCHER-02; workflow run 34196807435). Transport and paging fixed (§5.28). A pager exists as of 2026-09-11 and was drilled; remaining is the cron, which delivered 27% of the passes it was asked for. **Re-measured 2026-09-13 at 7% of a 15-minute cron — but 0.269 passes/hour against 0.27/hour when it was hourly, so the interval is an inert knob and detection latency is a ~3.3 h median with a 7.2 h worst case (§5.36).** And the row was measuring the wrong thing anyway: for four days after the redeploy `MONITOR_FACTORY`/`MONITOR_TREASURY` still named the **retired** pair, so the cadence of a monitor watching nothing was not the binding problem. Fixed, guarded by `WATCHER-06`, gap now stated by `WATCHER-05`. |
