...
| **PM-E4** | On-call roster placeholders replaced | `INCIDENT_RESPONSE.md` §1 has real handles | 🟡 the three Safe signers are named — Tom, Jack and Joe, each bound to the owner address they signature-proved on 2026-09-04 — so the roster is no longer hypothetical. What the acceptance criterion actually asks for is still missing: **a contact channel for any of them.** The column exists to answer "how do I wake two of these three at 03:00", and a name does not answer it. §8.2 timed the mechanical path at 5 s inside a 60-second budget, so the budget is almost entirely the human hop. Needs a phone or pager per signer, plus incident commander, comms and legal, which are untouched |
...
| **PM-C1** | ❌ | Mainnet deploy. The 46630 rehearsal is finished; this row is the 4663 run. Now gated by `preflightMainnet.mjs`, which verifies `.env.production` itself — including the check `DeployMainnet.s.sol` lacks, that `PLATFORM_TREASURY` has code at all. The remaining gate is two new EOAs that do not exist yet, not an underfunded testnet wallet; see the note under §3 and `C1_RUNBOOK.md`. |
| **PM-C2** | ❌ | Safe `acceptOwnership` on factory and treasury. The Safe exists (D4); this is blocked on C1. Mechanism rehearsed: a 2-of-3 Safe took and returned ownership of the testnet factory on 2026-09-04 (§8.2). |
...
| **PM-D4** | 🟡 | Safe created 2026-09-04: `0x2953957774482efA660921df85A1E7634ccfe27A`, 2-of-3 SafeL2 1.4.1 on 4663, indexed by the transaction service, all three owners signature-verified, `verifyOwnerSafe.mjs` green. This unblocks **C1**, which takes `PROD_OWNER_SAFE` as a required input, and supplies PM-C9's `PLATFORM_TREASURY`. Remaining: `INCIDENT_RESPONSE.md` §1 still lists the owners as addresses rather than reachable people. Q1's third criterion closed in §8.3. |
...
noticing before reading the count as reassuring: 7 open and 6 partial is a
...
- **PM-E2, delivery half** — the host and the sink now exist and were driven end
  ...
  schedule watches testnet until C1 supplies mainnet addresses, and a P0 lands
  in an issue rather than on a phone. The second half is the same missing piece
  as §1's contact column — a channel that pushes.
