...
| **PM-C4** | Contracts verified on the block explorer | Public verified source at the deployed address | ❌ **confirmed NOT verified, 2026-09-09, and `--verify` could never have worked.** Both `0xBa9d2E86…` and `0x99aD248d…` show a "Verify & publish" button and bytecode only — no source, no contract name, no Read/Write tabs. This row previously read as *probably done, needs confirming*, on the strength of `--verify` being in the broadcast command. It was not done, and the flag was never capable of doing it: every path under `https://robinhoodchain.blockscout.com/api` sits behind a Cloudflare **managed challenge**, so forge receives an HTML interstitial where it expects JSON and reports `Failed to deserialize response: expected value at line 1 column 1` — which reads like a malformed reply rather than a wall, and which scrolled past under the deployment output. Re-attempted 2026-09-09 and it fails identically, so this is a property of the explorer, not of that sitting. The HTML surface is **not** challenged, so the browser form works and is the only route. `scripts/genVerifyInput.mjs` produces exactly what it needs — standard JSON rather than flattened source, because `via_ir = true` has nowhere to live in the flattened flow and the explorer's recompile would mismatch on bytecode that is actually correct. `src/`, `foundry.toml` and the `lib/` pointers are byte-identical to deploy commit `d0220e2`, so those inputs still match the deployed bytecode. Testnet 46630 **is** verified, which is most of why nobody suspected the mainnet flag |
...
| **PM-C4** | ❌ | Explorer verification of the *mainnet* deploy at `0xBa9d2E86…` / `0x99aD248d…`. Checked 2026-09-09: both show "Verify & publish", i.e. not verified. `--verify` cannot reach this explorer — Cloudflare challenges the whole `/api` surface — so this is a browser-form task. Inputs: `node scripts/genVerifyInput.mjs`. Testnet 46630 is already verified. |
